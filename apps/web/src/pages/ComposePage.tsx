...
import ComposeWorkspaceToolbar from '../components/compose/ComposeWorkspaceToolbar';
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-[#f2f5f9]">
      ...
      <ComposeWorkspaceToolbar
        ...
        zoomFitsWidth={zoomMode === 'fit-width'}
        inspectorOpen={inspectorOpen}
        onUndo={undoWorkspace}
        onRedo={redoWorkspace}
        onClear={handleClearAll}
        onSave={handleSaveCurrent}
        onOpenHandout={() => openLessonRoute('/handout')}
        onToggleOutline={() => setOutlineOpen((current) => !current)}
        onOrganizeByKnowledge={handleOrganizeByKnowledge}
        onOpenRules={() => setBlueprintRulesOpen(true)}
        onFindSupplementCandidates={() => void handleFindSupplementCandidates()}
        onBuildTeachingBlueprint={handleBuildTeachingBlueprint}
        onAutoLayout={handleAutoLayout}
        onOpenTemplates={() => navigate('/templates?scope=compose')}
        onChangeOutputProfile={applyOutputProfile}
        onToggleAnswers={() => setShowAnswers((prev) => !prev)}
        onToggleAnalysis={() => setShowAnalysis((prev) => !prev)}
        onDecreaseZoom={() => nudgeZoom(-10)}
        onResetZoom={() => { setZoomMode('manual'); setDocumentZoom(100); }}
        onIncreaseZoom={() => nudgeZoom(10)}
        onFitWidth={() => setZoomMode('fit-width')}
        onToggleInspector={() => { setInspectorOpen((prev) => !prev); setInspectorTab('view'); }}
        onOpenSlides={() => openLessonRoute('/slides')}
        onOpenClassroom={() => openLessonRoute('/classroom')}
      />
      ...
      <div className="compose-workbench-ui relative flex min-h-0 flex-1">
        ...
        {/* Canvas */}
        <section aria-label="试卷画布" className="relative flex min-w-0 flex-1 flex-col bg-[#d9e2ed]">
