...
def test_image_catalog_preserves_legacy_path_and_reads_binding_metadata(tmp_path) -> None:
    ...
    app = FastAPI()
    app.include_router(build_image_catalog_router(ImageCatalogService(ImageCatalogRepository(str(db_path)))))

    response = TestClient(app).get("/images", params={"q": "force", "verified": "true"})
