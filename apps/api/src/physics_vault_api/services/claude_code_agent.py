...
- `mcp__physics_vault__add_knowledge_to_composition_workbench`：把标准 topic3_id 加入工作台作为知识卡；只写草稿，绝不修改知识目录。
...
- `mcp__physics_vault__apply_composition_workbench_plan`：组卷操作的优先接口。一次调用即可批量加入正式题库题目、标准知识卡、试卷标题/分节/教学文字，并用 ordered_refs 指定最终顺序。先 dry_run=true 返回完整预览，老师确认后才 dry_run=false；它只写组卷草稿，不修改题库或知识目录。
